    ...
    override fun apply(project: Project) = project.run {
        ...
        if (kotlinClass != null) {
            plugins.findPlugin(kotlinClass)?.run {
                ...
                if (VersionNumber.parse(kotlinPluginVersion) < VersionNumber(1, 5, 0, null))
                    logger.error("JetBrains Gradle Benchmarks plugin requires Kotlin version 1.5.0 or higher")
            }
        }
