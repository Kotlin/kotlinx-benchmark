...
class ProjectIsolationTest : GradleTest() {
    @Test
    fun testIsolation() {
        // Use either 2.3.20, or a Kotlin version used to build tests, if it is a more recent one
        // (which is the case when running tests with Kotlin built from the HEAD).
        val version = KotlinTestVersion.mostRecent(
            KotlinTestVersion.Kotlin2_3_20.versionString,
            KotlinTestVersion.overriddenVersion() ?: KotlinTestVersion.Kotlin2_3_20.versionString
        )
