...
directory (or `android<configurationName>` for non-main configurations).
...
Due to restrictions in Jetpack Microbenchmarks, not all `BenchmarkConfiguration`
options are supported on Android:
...
- `BenchmarkConfiguration.warmups`
- `BenchmarkConfiguration.iterationTime`
- `BenchmarkConfiguration.iterationTimeUnit`
